    ...
    for (idpos in 1:monCount) {
        ...
        my_noNaDs <-
            my_dataset[!is.na(my_dataset[,1]) &
                           !is.na(my_dataset[,2]) &
                           !is.na(my_dataset[,3]) & !is.na(my_dataset[,4]),]
        lessLen <- length(my_noNaDs[,1])
        # print ( paste("without NA:", lessLen ) )
        nobs[idpos] <- lessLen
    }
    myMatrix <- cbind(id,nobs)
    myMatrix
}